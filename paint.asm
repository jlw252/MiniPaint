...
drawmode_nothing:
	...
	#Check if user is presses KEY_F
	display_is_key_pressed a0, KEY_F
	beq a0, 0, _not_KEYF
	jal flood_fill
	...
	_next:	
		...
		#Set last_x and last_y to the current mouse coordinates
...
check_palette_click:
	...
	#Check if the mouse coordinates are within the palette area
   	blt t2, PALETTE_X, _end_check_palette_click #less than or = 
    bge t2, PALETTE_X2, _end_check_palette_click #greater than or =
    
    _not_in_palette:
		sub t2, t2, PALETTE_X
		sub t3, t3, PALETTE_Y
	
		#Calculate the color based on palette position
		div t3, t3, 4
		div t2, t2, 4
...
flood_fill:
	...
	#Call display_get_pixel to get color
	jal display_get_pixel
	
	#Call flood_fill_rec with current mouse coordinates, color, drawing
	move a2, v0
	lw a3, color
	lw a0, display_mouse_x
	lw a1, display_mouse_y
...
flood_fill_rec:
	...
	move s0, a0 #x
	move s1, a1 #y
	move s2, a2 #target color
	move s3, a3 #drawing color
	
	#Get color of pixel
	#x, y passed into function
	move a0, s0
	move a1, s1
	jal display_get_pixel
	
	#Check if pixel is already the replacement color
	beq v0, s3, _return
	
	#Check if pixel is not target color
	bne v0, s2, _return
